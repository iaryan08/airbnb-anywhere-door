      ...
      try {
        ...
        if (searchRes.ok) {
          ...
          searchContext = `[Real-Time Grounding Information from Web Search]:\n` +
            ...
            `Search Results: ${JSON.stringify((searchData.results || []).slice(0, 3).map((r: { title: string; content: string; url: string }) => ({ title: r.title, content: r.content, url: r.url })))}`;
          tavilyUsed = true;
          console.log("Tavily grounding retrieved successfully.");
        } else {
  ...
  "listings": [
    {
      ...
      "price": "${currency}X,XXX/night (or /person or /session for non-stays)",
      "rating": number between 4.5 and 5.0,
      "highlights": "Two vivid, specific sentences that sell the place. Mention unique details — the view, a smell, a feeling — not generic amenities.",
...
HARD RULES:
- Listings count: always 3. Itinerary days: match the requested duration (default 3 if unspecified). Budget: always complete.
...
- For day trips / no-stay requests: listings = local experiences/activities (price as /person or /ticket), accommodation budget = ${currency}0.
